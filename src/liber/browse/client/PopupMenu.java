...
public class PopupMenu extends PopupPanel
{
		...
		setWidget(menu);
   		...
   		else
   		{
			...
			if (ai.getURI() != null)	//if this anchor is a downloadable resource, add an option
   			{
   				w.setURI(ai.getURI(), ai.getWords());
	   			menu.addItem("Download", true, new DownloadCommand(ai.getURI(), ai.getWords(), this));
   			}
	   		//if (w instanceof BrowsingTab)
	   		//	menu.addItem("Add to 'Describe your resource' panel", true, new ExportCommand(ai.getID(), w, parent, this));			
  	...
  	/**	Create a editing or browsing command. This method ensures all commands for the modules

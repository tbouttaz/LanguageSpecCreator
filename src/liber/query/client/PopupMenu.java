...
public class PopupMenu extends PopupPanel
{
  	...
  	public PopupMenu(AnchorInfo ai, WysiwymTab w, WYSIWYMinterface parent) 
   	{
		...
		if (ai.isSummation())	//if this is a summation anchor, only add one option
		...
		else if (w instanceof BrowsingTab)	//add an item for specifying that something is somehow connected 
 		{
 		...
 		{
	 		menu.addItem("Any relationship", true, new QueryCommand(null, null, ai.getID(), w, parent, this));
	...
	/**	Create a editing, browsing or querying command. This method ensures all commands for the modules
  	 ...
  	 *	@param parent Entrypoint
  	 */
  	private WysiwymCommand createCommand(String prop, String nl, String id, WysiwymTab w, WYSIWYMinterface parent)
  	{
  		if (w instanceof QueryTab)
  			return new QueryCommand(prop, nl, id, w, parent, this);
  		else if (w instanceof BrowsingTab)
  			return new BrowseCommand(prop, nl, id, w, parent, this);
   		else
  			return new WysiwymCommand(prop, nl, id, w, parent, this);
  	}
}
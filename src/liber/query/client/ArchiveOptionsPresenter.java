 ...
 *	see an object's complete descriptions in a browsing pane. Developed for editing module, should
 *	not be used in querying module.
 *
 ...
 */
 public class ArchiveOptionsPresenter extends InputPopupPanel implements ClickListener
 {
 	...
 	private boolean login = false, automatic = false;
 	private Help help;
	...
	private void init(String message)
	{
    	...
    	for (int i = 0; i < radio.length; i++)
    	{
    		...
    		vp.add(radio[i]);
			radio[i].addClickListener(new optionClickListener(options[(i*2)], user, parent, login));
    	}
 		...
 		if (sender == yesBtn)
 		{
 			for (int i = 0; i < radio.length; i++)
    		{
    			if (radio[i].isChecked())
    			{
    				...
    				Window.alert("why use archive options presenter in query module");
 	...
 	private class optionClickListener implements ClickListener
	{
    	...
    	public optionClickListener(String str, String u, WYSIWYMinterface p, boolean l)
    	{
    		id = str;
    		user = u;
    		parent = p;

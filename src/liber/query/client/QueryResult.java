...
/**
 * 	Displays an overview of the search results, with 'more' links for the
 *	user to request a full description in a BrowsingBox.
 *
 *	Used by previous version of LIBER; current version displays feedback-text
 *	description of all search results in BrowsingBox immediately.
 *	
...
public class QueryResult extends MyDialogBox
{
	...
	public QueryResult(AnchorInfo[][] results, WYSIWYMinterface p, QueryTab qt)
  	{
  		super(false, false);
  		setHeight(Window.getClientHeight() - 110);
  		setWidth(Window.getClientWidth() - 110);
  		parentWidget = p;
  		tab = qt;
